...
$SafeRemote = "/home/u378554361/domains/fabai.fableadtech.in/public_html"
$RemoteDir = $RemoteDir.TrimEnd('\', '/')
if ($RemoteDir -match 'Program Files') {
  $RemoteDir = $SafeRemote
}
if ($RemoteDir -ne $SafeRemote) {
  Write-Host "REFUSED: remote path must be exactly: $SafeRemote" -ForegroundColor Red
  Write-Host "Got: $RemoteDir" -ForegroundColor Red
  exit 1
}
