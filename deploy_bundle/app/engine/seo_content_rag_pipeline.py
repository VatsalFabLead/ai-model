...
from app.engine.seo_optimizer_engine import analyze_keyword_density, improve_readability, readability_score
...
from app.engine.seo_content_enrichment import (
  ...
  generate_image_placeholders_with_alt,
  generate_trust_and_citation_blocks,
...
def run_seo_optimization_stage(
  ...
  *,
  ...
  slug = draft.get("slug") or re.sub(r"[^\w\s-]", "", topic.lower()).replace(" ", "-")
  schema = build_full_schema(
    title, meta_desc, article, faqs, topic, slug,
  ...
  # Calculate E-E-A-T & Keyword Density Metrics
  eeat = compute_eeat_score(article, facts, float(draft.get("confidence") or 0.8), r_score, sources_used)
  kw_density = analyze_keyword_density(article, keywords)
  ...
  draft["gaps"] = gaps
  draft["eeat"] = eeat
  draft["keyword_density"] = kw_density
  ...
  return {
    ...
    "primary_keyword_density": kw_density.get("primary_density"),
